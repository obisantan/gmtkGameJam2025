...
func load_words(path: String) -> void:
	all_words.clear()
	...
	while not file.eof_reached():
		var word: String = file.get_line().strip_edges().to_lower()
...
func has_valid_loop(words: Array[String], min_length: int = 3) -> bool:
	...
	for start_word in graph.keys():
		var stack: Array[Variant] = [[start_word, [start_word]]]
...
func create_valid_loop(loop_graph: Dictionary, loop_size: int) -> Array[String]:
	var candidates: Array = loop_graph.keys()
	var tries: int = 100
...
func get_level_words() -> Array[String]:
	...
	while loops_found < target_loop_count and loop_attempts > 0 and slots_remaining > 0:
		var loop_size: int = randi_range(loop_size_range.x, loop_size_range.y)
		var loop: Array[String] = create_valid_loop(full_word_graph, loop_size)

		if loop.is_empty():
			loop_attempts -= 1
			continue

		var new_words: Array = loop.filter(func(w): return not used_words.has(w))
